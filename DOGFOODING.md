...
3. **Push through difficulties; a failed first attempt is a routing problem, not a verdict.** Expect the obvious installers to fail and route around them:
   ...
   - `git push` and PR creation via the default proxy/MCP may `403`; push and open PRs with `https://x-access-token:$GH_TOKEN@github.com/<owner>/<repo>.git` and the GitHub REST API. The token URL alone is not always enough — the `403` can come from the proxy rather than from GitHub, in which case the request must also leave the proxy: add the GitHub hosts to `no_proxy` and clear git's proxy for the call (`git -c http.proxy= -c https.proxy= push …`), and pass `--noproxy '*'` to `curl` for REST calls. Keep TLS verification on and do not unset `HTTPS_PROXY` globally.
